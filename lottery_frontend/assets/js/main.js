...
function changedDay(obj){
    var value = obj.value;
    var today1 = new Date();
    var arr = ['Chủ nhật','Thứ 2','Thứ 3','Thứ 4','Thứ 5','Thứ 6','Thứ 7'];
    var arrCurrentDay=[];
    var arrDay=[];
    var arrMonth=[];
    var arrYear=[];
    if(checkLink(resultLotteryAPi)==false){
        for(let i =0; i<arr.length;i++){
            today1.setDate(today1.getDate()-1);
            arrCurrentDay.push(today1.getDay());
            arrDay.push(today1.getDate());
            arrMonth.push(today1.getMonth()+1);
            arrYear.push(today1.getFullYear());
        }
    }else{
        arrCurrentDay.push(today1.getDay());
        arrDay.push(today1.getDate());
    ...
    for(let i=0; i<arrCurrentDay.length;i++){
        if(value==arrCurrentDay[i]){
            resultLotteryAPi='http://localhost:8081/Lottery/Date/'+arrDay[i]+'-'+arrMonth[i]+'-'+arrYear[i];
            resetTable()
            loading();
            start();
        }
...
function search() {
    ...
    || (year == nowYear && month == nowMonth && date > nowDate)){
        alert('Ngày nhập vào đang lớn hơn ngày hiện tại')    
    }else if(checkLink('http://localhost:8081/Lottery/Date/'+date+'-'+month+'-'+year)===false){
        alert('Không có dữ liệu')
    }else{resultLotteryAPi='http://localhost:8081/Lottery/Date/'+date+'-'+month+'-'+year;
    // alert(checkLink(resultLotteryAPi))
    resetTable();
    loading();
    start();
}
...
function renderTitleTable(results){
    ...
    titleTable.innerHTML = '<th style="padding: 12px;" colspan="'+col+'"><h2>Kết quả Xổ số Miền Nam - '+date+'</h2></th>';
    }
    function resetTable(){
